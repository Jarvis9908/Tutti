...
void usage(const char* prog) {
    std::fprintf(stderr,
        "Usage:\n"
        "  IN_PROCESS: %s [--gpu N] [--cap N] <PCI_BDF> [<PCI_BDF>...]\n"
        ...
        "  e.g. (service): %s --cuda 0 --service 127.0.0.1:50051 --dev-id 0\n"
        "Exercises tutti::HostDeviceMemorySubsystem against one or more\n"
        "Devices, either IN_PROCESS (LocalNvmeDirectRegistry -- daemon\n"
        "MUST NOT be running) or SERVICE_CLIENT (a running tutti_daemon).\n"
...
int run(const tutti_examples::RegistryCliOptions& ropt) {
    prime_cuda(ropt.cuda_dev);
    const int cuda_dev = ropt.cuda_dev;

    // [1] Bring up every requested Device -- IN_PROCESS or SERVICE_CLIENT.
    ...
    STEP_OK("registry up: mode=%s n=%zu", ropt.service_mode ? "service" : "direct",
            devices.size());
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ) {
        ...
        if (std::strcmp(a, "--gpu") == 0 && i + 1 < argc) {
            ropt.cuda_dev = std::atoi(argv[i + 1]); i += 2; continue;
        }
        if (std::strcmp(a, "-h") == 0 || std::strcmp(a, "--help") == 0) { usage(argv[0]); return 0; }
        if (a[0] == '-') { usage(argv[0]); return 1; }
        ropt.pci_addrs.emplace_back(argv[i++]);
    }
