 ...
 *   [6]  open_gpu_file(CREATE) 2 GpuFiles with tensor_shape=[2,4,4096]
...
void usage(const char* prog) {
    std::fprintf(stderr,
        "Usage:\n"
        "  IN_PROCESS: %s [--gpu N] [--cap N] <PCI_BDF> <PCI_BDF> [<PCI_BDF>...]\n"
...
bool do_tensor_io(tutti::INvmeStorage& storage,
                  ...
                  bool                 do_verify)
{
    AlignedBuf wbuf(kTensorSize);
    AlignedBuf rbuf(kTensorSize);
...
int run(const tutti_examples::RegistryCliOptions& ropt)
{
    ...
    for (uint32_t i = 0; i < kNumGpuFiles; ++i) {
        ...
        if (gf == nullptr)
            STEP_FAIL("open_gpu_file(CREATE) '%s'", nm.c_str());
    ...
    STEP_OK("open_gpu_file(CREATE) + write %u GpuFile(s) "
    ...
    for (uint32_t i = 0; i < kNumGpuFiles; ++i) {
        ...
        tutti::GpuFileSpec spec{};
        spec.name = nm;
        tutti::GpuFile* gf = bs->open_gpu_file(spec);   // EXISTING
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ) {
        ...
        if (a == "--gpu" && i + 1 < argc) { ropt.cuda_dev = std::atoi(argv[i + 1]); i += 2; continue; }
        ropt.pci_addrs.push_back(std::move(a));
        ++i;
    }
