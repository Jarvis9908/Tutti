...
int main(int argc, char** argv) {
    ...
    for (std::size_t i = 0; i < N; ++i) {
        ...
        if (nf->extents.empty()) STEP_FAIL("open_file(CREATE,%s) empty extents", nm);
        files.push_back(nf);
    }
    STEP_OK("open_file(CREATE) 1 MiB on %zu device(s)", N);

    // [5] host-side write known pattern.  Use a pattern that's
    //     unique per device + per byte so a buffer mix-up would be
    //     obvious.  O_DIRECT requires block-aligned buffers.
    std::vector<uint8_t*> patterns(N, nullptr);
    for (std::size_t i = 0; i < N; ++i) {
        if (::posix_memalign(reinterpret_cast<void**>(&patterns[i]),
                             4096, kIoBytes) != 0)
            STEP_FAIL("posix_memalign(patterns[%zu])", i);
    ...
    //     (implicitly, since default-stream work is already ordered).
    if (!storage->configure_handle_pool((uint32_t)N, (uint32_t)N))
        STEP_FAIL("configure_handle_pool(%zu, %zu)", N, N);
    std::vector<tutti::NvmeFileDeviceHandle*> dhs(N, nullptr);
    for (std::size_t i = 0; i < N; ++i) {
        dhs[i] = storage->acquire_device_handle(files[i], /*stream=*/nullptr);
        if (dhs[i] == nullptr) STEP_FAIL("acquire_device_handle(file[%zu])", i);
    }
    STEP_OK("configure_handle_pool + acquire_device_handle per file (n=%zu)", N);
    ...
    // Free aligned pattern buffers (no longer needed after byte-compare).
    for (std::size_t i = 0; i < N; ++i) ::free(patterns[i]);

    // [11] tear down per-device GPU buffers + handles
    for (std::size_t i = 0; i < N; ++i) {
        storage->release_device_handle(files[i], /*stream=*/nullptr);
