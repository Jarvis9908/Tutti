 ...
 *   - The CAP_SUBMIT_* bits are the cross-process contract for
 *     advertising which io_engine submission paths a Device supports;
 *     the io_engine implementations themselves carry their own
 *     interface (see io_engine/include/io_engine.h).
 *
 * Bit-layout rules:
 ...
 *
 * Submission modes (bits 0..15) advertise which io_engine
 * submission entrypoints the Device supports; a Device that
 * advertises e.g. CAP_SUBMIT_BATCH_GPU_STREAM is contractually
 * required to accept the matching submit_batch path through its
 * IIoEngine wrapper.
 */
...
/**
 * Capability rollup for one Device or one io_engine instance.
 * Read-only after the device is brought up.
 *
...
struct CapabilitySet {
    ...
    std::size_t max_batch_count;    // largest batch count
    std::size_t max_queue_depth;    // largest SQ/CQ depth
